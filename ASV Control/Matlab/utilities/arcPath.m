function [yawRef, ASV] = arcPath(ASV, ref)
    ...
    % spoof location as if circle was centred on [0,0]
    ...
    %% Error
    % cross track error
    % desired position (arc centred on [0,0])
    xD = r*cosd(theta);
    yD = r*sind(theta);
    % absolute value of cross track error
    ASV.error_crossTrack = sqrt((xD - xSpoof)^2 + (yD - ySpoof)^2);
    
    % should cross track error be +ve or -ve?
    % check if above or below arc
    if ASV.Y < yD
        % below, increase yaw, negative cross track error
    ...
    %% Integral
    % add integral of previous step to persistent value
    ...
    %% Coordination state
    % calculate current gamma value, normalised over a 180 deg arc
    if ref.pathType == 2 % clockwise arc
