function [Quad] = quad_variables(sim, vehicleID, initialPosition)
    ...
    Quad.X_KD = -0.35;         % KD value in X position control
    Quad.X_KI_lim = 0.25;         % Error to start calculating integral term

    Quad.Y_KP = Quad.X_KP;          % KP value in Y position control
    Quad.Y_KI = Quad.X_KI;            % KI value in Y position control
    Quad.Y_KD = Quad.X_KD;         % KD value in Y position control
    Quad.Y_KI_lim = Quad.X_KI_lim;         % Error to start calculating integral term
